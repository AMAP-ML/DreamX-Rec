...
class IntHQDemo(nn.Module):
    ...
    def profile_vector(self, h0, positions):

        index = positions.unsqueeze(-1).expand(-1, -1, h0.size(-1))
        emb = h0.gather(1, index)
    ...
    def forward(self, batch):
        ...
        profile_vec = self.profile_vector(h0, batch["ctx"]["profile_positions"])
    ...
    def _poi_base_valid(self, batch, name, entry):
        return (entry["pos_id"] != -1) & self._task_slot_valid(batch, name)
    ...
    def loss_components(self, batch):
        ...
        for name in TASKS:
            if name in POI_TASKS:
                entry = labels[name]
                valid = self._poi_base_valid(batch, name, entry)
                train_mask = valid & ~self._first_valid(valid)
                all_logits, _, _ = self.poi_scores(name, out, entry, return_ids=True)
                logits = all_logits[train_mask]
                count = train_mask.sum()
                if logits.size(0):
                    target = torch.zeros(
                        logits.size(0), dtype=torch.long, device=logits.device
                    )
                    loss_sums[name] = F.cross_entropy(logits, target, reduction="sum")
                    rank = (logits > logits[:, :1]).sum(-1)
                    metrics[name + "/hr@1"] = (rank < 1).float().mean().item()
                else:
                    loss_sums[name] = all_logits.reshape(-1)[0] * 0
            else:
                label = labels[name]
                valid = (label != -1) & self._task_slot_valid(batch, name)
                train_mask = valid & ~self._first_valid(valid)
                all_logits = self.cls_logits(name, out[name])
                logits = all_logits[train_mask]
                count = train_mask.sum()
                if logits.size(0):
                    ...
                    loss_sums[name] = all_logits.sum() * 0
            counts[name] = count

        return loss_sums, counts, metrics

    def loss(self, batch):
        loss_sums, counts, metrics = self.loss_components(batch)
        losses = [loss_sums[name] / counts[name].clamp(min=1) for name in TASKS]
    ...
    def evaluate(self, batch):
        ...
        for name in POI_TASKS:
            entry = labels[name]
            eval_mask = self._first_valid(self._poi_base_valid(batch, name, entry))
        ...
        for name, topk in (("how", 3), ("when", None)):
            label = labels[name]
            valid = (label != -1) & self._task_slot_valid(batch, name)
            eval_mask = self._first_valid(valid)
            if not bool(eval_mask.any()):
                continue
    ...
    def eval_tensors(self, batch):
        ...
        for name in POI_TASKS:
            entry = labels[name]
            valid = self._first_valid(self._poi_base_valid(batch, name, entry))
            logits, category = self.poi_scores(name, out, entry)
            res[name] = {
                "prob": logits,
                "mask": valid,
